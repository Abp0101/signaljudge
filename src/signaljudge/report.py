...
const corrected=data.cases.filter(c=>c.corrected_model_only||c.corrected_market_only);document.getElementById('cases').innerHTML=corrected.map(c=>`<div class="case"><b>${esc(c.selection)}</b><div class="small">${esc(c.event_id)}</div><div class="yes">✓ Corrected ${c.corrected_model_only?'model-only':'market-only'} baseline</div></div>`).join('')||'<div class="small">No corrected classifications in this replay.</div>';
