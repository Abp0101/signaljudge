...
<script>
    ...
    const metricEntries=Object.entries(data.metrics);if(metricEntries.length){const best=Math.min(...metricEntries.map(([,x])=>x.brier));document.getElementById('metrics').innerHTML=metricEntries.map(([name,m])=>`<div class="card ${m.brier===best?'winner-card':''}"><div class="metric-title"><span>${name==='AGENT'?'SignalJudge':name+' only'}</span><span>${m.brier===best?'BEST':''}</span></div><div class="bar"><span style="width:${Math.max(5,(1-m.brier)*100)}%"></span></div><div><b>Brier ${m.brier.toFixed(3)}</b> · Log loss ${m.log_loss.toFixed(3)}</div><div class="small">Accuracy ${pct(m.accuracy)} · n=${m.sample_size}</div></div>`).join('');}else{document.getElementById('metrics-panel').hidden=true;}
