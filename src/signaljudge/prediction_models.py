...
from signaljudge.rating import DEFAULT_RATING, elo_outcome_probabilities
...
def _validated_team_state(
    ...
    return ratings, appearances, aliases


def _bounded_parameter(
    parameters: Mapping[str, Any], name: str, low: float, high: float
) -> float:
    try:
...
def _validated_metrics(
    parameters: Mapping[str, Any], raw_metrics: Mapping[str, Any]
) -> Tuple[ModelMetrics, int]:
    try:
...
def _validated_sources(raw_sources: Any) -> List[Dict[str, Any]]:
...
class RatingModel:
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "RatingModel":
        ...
        ratings, appearances, aliases = _validated_team_state(payload)
        ...
        metrics, minimum_team_games = _validated_metrics(parameters, raw_metrics)
        ...
        return cls(
            ...
            aliases=aliases,
            home_advantage=_bounded_parameter(
                parameters, "home_advantage", -200.0, 300.0
            ),
            draw_base=_bounded_parameter(parameters, "draw_base", 0.05, 0.60),
            draw_scale=_bounded_parameter(
                parameters, "draw_scale", 100.0, 10_000.0
            ),
            minimum_draw=_bounded_parameter(parameters, "minimum_draw", 0.01, 0.50),
            maximum_draw=_bounded_parameter(parameters, "maximum_draw", 0.01, 0.60),
    ...
    def predict(self, fixtures: List[Fixture], generated_at: datetime) -> List[Prediction]:
        ...
        for fixture in fixtures:
            ...
            probabilities = elo_outcome_probabilities(
                self.ratings.get(home_key, DEFAULT_RATING),
                self.ratings.get(away_key, DEFAULT_RATING),
                self.home_advantage,
                self.draw_base,
                self.draw_scale,
                self.minimum_draw,
                self.maximum_draw,
                three_way=self.outcome_mode == "three_way",
            )
            outcome_probabilities = {
                fixture.home_team: probabilities["H"],
                fixture.away_team: probabilities["A"],
            }
            if "D" in probabilities:
                outcome_probabilities["Draw"] = probabilities["D"]
