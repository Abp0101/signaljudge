...
const age=value=>{const seconds=Number(value);if(!Number.isFinite(seconds))return '—';if(seconds<60)return `${Math.round(seconds)}s`;if(seconds<3600)return `${Math.round(seconds/60)}m`;return `${(seconds/3600).toFixed(1)}h`};
...
function renderSummary(data){const source=data.source_counts||{},model=data.prediction_source||{},quota=data.market_source?.quota||{},evidence=(data.matches||[]).filter(item=>item.valid_book_count>0),averageBooks=evidence.length?evidence.reduce((sum,item)=>sum+Number(item.valid_book_count||0),0)/evidence.length:null;const cards=[['Fixtures',data.total_events,'Fetched provider events'],['Reconciled',data.reconciled_events,'Independent prediction matched'],['Conflicts',data.material_conflicts,'Probability or rank gap'],['Model wins',source.MODEL||0,'Explicit source selection'],['Market wins',source.MARKET||0,'Explicit source selection']];if(averageBooks!==null)cards.push(['Market books',averageBooks.toFixed(1),`${String(data.region||'').toUpperCase()} region average`]);if(model.accuracy!==undefined)cards.push(['Model test',pct(model.accuracy),`${number(model.sample_size)} held-out matches`]);cards.push(['API quota',quota.remaining??'—','Requests remaining']);el('summary').innerHTML=cards.map(card=>`<article class="card"><div class="card-label">${esc(card[0])}</div><div class="card-value">${esc(card[1])}</div><div class="card-note">${esc(card[2])}</div></article>`).join('')}
...
function renderRows(){const rows=visibleRows();el('empty').hidden=rows.length>0;el('ranking').innerHTML=rows.map(item=>{const winner=String(item.winner||'UNAVAILABLE').toLowerCase();const eventState=String(item.event_state||'UNKNOWN').toLowerCase();const opponent=item.selection===item.home_team?item.away_team:item.selection===item.away_team?item.home_team:item.selection==='Draw'?'Home and away teams':'—';const conflict=item.material_conflict?'<div class="small conflict">Material conflict</div>':'';const reasons=(item.reason_codes||[]).map(esc).join(' · ');const books=(item.bookmakers||[]).map(esc).join(' · ');const rejected=(item.rejected_books||[]).map(esc).join(' · ');const evidence=item.market_probability===null||item.market_probability===undefined?'':`<div class="small">${number(item.valid_book_count)}/${number(item.total_book_count)} books · median age ${age(item.market_median_age_seconds)} · dispersion ${pct(item.market_dispersion)}</div><div class="small book-list">Books: ${books||'—'}${rejected?` · Rejected: ${rejected}`:''}</div>`;return `<tr><td class="rank">${esc(item.final_rank??'—')}</td><td class="fixture"><b>${esc(item.home_team)} vs ${esc(item.away_team)}</b><div class="small">Home vs away · ${esc(item.sport_key)} · ${esc(item.event_id)}</div><span class="pill ${esc(eventState)}">${esc(item.event_state)}</span></td><td class="kickoff">${esc(dateTime(item.commence_time))}</td><td class="selection"><b>${esc(item.selection||'Prediction unavailable')}</b><div class="small">Opponent: ${esc(opponent)}</div></td><td class="pct">${pct(item.model_probability)}</td><td class="pct">${pct(item.market_probability)}${conflict}${evidence}</td><td class="pct">${pct(item.reconciled_probability)}</td><td><span class="pill ${esc(winner)}">${esc(item.winner)}</span><div class="small">${item.decision_confidence===null||item.decision_confidence===undefined?'':`${pct(item.decision_confidence)} confidence`}</div></td><td class="reason">${esc(item.rationale)}<div class="small">${reasons}</div></td></tr>`}).join('')}
