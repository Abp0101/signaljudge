...
function statusMessage(data){const p=data.prediction_source||{};if(data.market_source?.degraded)return `Provider unavailable: showing explicitly degraded cache (${Math.round(data.market_source.cache_age_seconds)}s old).`;if(state.mode==='live'&&p.status==='missing')return `Live fixtures loaded, but ${data.sport_key}.json is missing from the predictions directory. Fixtures remain visible without invented scores.`;if(state.mode==='live'&&data.reconciled_events===0)return `No prediction event IDs matched this live snapshot. ${p.loaded||0} prediction records were loaded and ${p.unmatched||0} did not match.`;return data.response_cache?'Using the recent in-process snapshot to protect API quota. Use Refresh now for a provider call.':''}
function setStatus(text,kind){const node=el('source-status');node.textContent=text;node.className=`status ${kind}`}
function render(){const data=state.data;if(!data)return;const origin=data.market_source?.origin||'UNKNOWN';setStatus(data.market_source?.degraded?`DEGRADED ${origin}`:`${origin} DATA`,data.market_source?.degraded?'warning':'good');el('board-eyebrow').textContent=state.mode==='demo'?'GUARANTEED REPRODUCIBLE RUN':'CURRENT PROVIDER SNAPSHOT';el('board-title').textContent=data.title;const source=data.prediction_source||{};const fetched=data.fetched_at?dateTime(data.fetched_at):'No reconciled run yet';el('board-meta').textContent=`${fetched} · ${data.region.toUpperCase()} region · ${source.loaded||0} independent predictions loaded`;const audit=data.audit||{};el('audit').textContent=`${audit.valid?'✓':'⚠'} Audit ${audit.valid?'verified':'invalid'} · ${number(audit.entries)} decisions`;el('audit').className=`audit ${audit.valid?'good':'bad'}`;renderSummary(data);renderRows();el('run-id').textContent=data.run_id?`Run ${data.run_id}${data.reused?' · idempotent replay':''}`:'No reconciliation run was created because no model prediction matched.'}
function renderSummary(data){const source=data.source_counts||{};const quota=data.market_source?.quota||{};const cards=[['Fixtures',data.total_events,'Fetched provider events'],['Reconciled',data.reconciled_events,'Independent prediction matched'],['Conflicts',data.material_conflicts,'Probability or rank gap'],['Model wins',source.MODEL||0,'Explicit source selection'],['Market wins',source.MARKET||0,'Explicit source selection'],['API quota',quota.remaining??'—','Requests remaining']];el('summary').innerHTML=cards.map(card=>`<article class="card"><div class="card-label">${esc(card[0])}</div><div class="card-value">${esc(card[1])}</div><div class="card-note">${esc(card[2])}</div></article>`).join('')}
